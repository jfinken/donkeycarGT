...
void ManagedTaskStatus::pause_task_blocking()
{
    ...
    if (m_state == MTSSM_PAUSED){
        printf("%s task is already paused\n", m_task_name.c_str());
        return;
    }
...
void ManagedTaskStatus::resume_task_blocking()
{
    ...
    {
        printf("%s task is already resumed\n", m_task_name.c_str());
        return;
    }
...
{
    concurrency::Lock lock(m_mutex);
    // if a supervisor is not interfering then skip
    ...
    // if already paused or a supervisor wants to resume then skip
    if (m_state == MTSSM_REQUEST_PAUSE)
    {
        printf("%s task pausing\n", m_task_name.c_str());
        m_state = MTSSM_PAUSED;
        m_event.trigger_all(); // inform the supervisor
        return false;
    }
    // wait until a supervisor wants to resume
