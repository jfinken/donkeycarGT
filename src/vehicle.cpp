...
void Vehicle::start_blocking()
{
    ...
    while (m_running) {
        update_parts();
        //  throttle the run loop via a max_loop_count or sleepy-time per desired frequency
    }
}
...
*
* PartIO requirements
*   1. A Part takes no input but produces an output (camera)
*   2. A Part takes input and produces output (image-conversion)
*   3. A Part takes an input but produces no output (mqtt image publisher)
*/
