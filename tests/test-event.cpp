...
TEST_CASE("basic event test") {
    ...
    std::thread t([&mutex, &event]() {
        ...
        event.wait(mutex);
        std::cerr << "[test-event] waiter thread signaled via event."
                  << std::endl;
    ...
    {
        std::cerr << "[test-event] main thread sleeping for " << sleep
                  << " ms ..." << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(sleep));
        {
            concurrency::Lock lock(mutex);
            std::cerr << "[test-event] main thread triggering all via event."
                      << std::endl;
        }
        // release t
        event.trigger_all();
